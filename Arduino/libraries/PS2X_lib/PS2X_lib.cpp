...
boolean PS2X::read_gamepad(boolean motor1, byte motor2) {
   ...
   for (byte RetryCnt = 0; RetryCnt < 5; RetryCnt++) {
      ...
      if ((/*(PS2data[1] & 0xf0) == 0x40 || */(PS2data[1] & 0xf0) == 0x70 || (PS2data[1] & 0xf0) == 0x20) && PS2data[2] != 0xfe /* || PS2data[1]  == 0x1*/)
         break;
   ...
   if (!(/*(PS2data[1] & 0xf0) == 0x40 || */(PS2data[1] & 0xf0) == 0x70 || (PS2data[1] & 0xf0) == 0x20/* || PS2data[1]  == 0x1 */ ) || PS2data[2] == 0xfe) {
      if (read_delay < 10)
...
byte PS2X::config_gamepad(uint8_t clk, uint8_t cmd, uint8_t att, uint8_t dat, bool pressures, bool rumble) {
  ...
  if(PS2data[1] != 0x41 && PS2data[1] != 0x23 && PS2data[1] != 0x73 && PS2data[1] != 0x79 && PS2data[1] != 0x47){
#ifdef PS2X_DEBUG
    Serial.println("Controller mode not matched or no controller found");
    Serial.print("Expected 0x41, 0x73 or 0x79, 0x47, but got ");
  ...
  for(int y = 0; y <= 10; y++) {
    ...
    if(PS2data[1] == 0x73 || PS2data[1] == 0x23 /*  || PS2data[1] == 0x41*/ )
      break;
